...
function ExperienceForm() {
  ...
  const fetchExperience = async () => {
    try {
      ...
      const response = await fetch(
        API_BASE_URL + `/experiences/get/${experienceId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      await fetch(API_BASE_URL + `/experiences/edit/${experienceId}`, {
        ...
        .then((response) => {
          if (response.status == 200) {
            ...
            setDataSnackbar({
              message:
                "Sorry, your experience couldn't be updated. Please try again later.",
              severity: "error",
            });
          }
      ...
      await fetch(API_BASE_URL + "/experiences/add", {
        ...
        .then((response) => {
          if (response.status == 201) {
            ...
            setDataSnackbar({
              message:
                "Sorry, your experience couldn't be sent. Please try again later.",
              severity: "error",
            });
          }
          return response.json();
  ...
  const formatDate = (date) => {
    ...
    return `Editing experience you created on ${dateObject.toLocaleDateString(
      "en-GB",
      options
    )}`;
  };
  ...
  return (
    <Grid>
      {isEditing() ? (
        ...
        <Typography>
          Fill in the details about your experience below.
        </Typography>
      )}
      ...
      <FormControl margin="normal" fullWidth>
        ...
        <Select
          value={departureStop}
          onChange={handleDepartureStopChange}
          disabled={lineNotEmpty && !isEditing()}
          onBlur={checkDepartureEmpty}
        >
      ...
      <FormControl margin="normal" fullWidth>
        ...
        <Select
          value={arrivalStop}
          onChange={handleArrivalStopChange}
          disabled={lineNotEmpty && !isEditing()}
          onBlur={checkArrivalEmpty}
        >
          {MenuItemsSelectedLineStops}
        </Select>
      </FormControl>
          ...
          <Button
            ...
            disabled={
              (lineNotEmpty ||
                ...
                durationNegative ||
                crowdednessNotEmpty ||
                satisfactionNotEmpty) &&
              !isEditing()
            }
          >
            {isEditing() ? "Edit experiemce" : "Share experience"}
          </Button>
        </Grid>
      </Grid>
    </Grid>
